...
import { Check, Pencil, Plus, RefreshCw, Save, Trash2 } from 'lucide-vue-next'
...
const selectedPet = computed(() => store.currentPet)
...
async function createPet() {
  ...
  try {
    ...
    error.value = exception instanceof Error ? exception.message : '创建失败'
  } finally {
    saving.value = false
  }
}
...
async function updatePet() {
  ...
  saving.value = true
  try {
    await store.updatePet(selectedPet.value.id, {
      ...editForm,
...
async function removePet() {
  ...
  const ok = window.confirm(`确定删除宠物「${selectedPet.value.name}」吗？相关设备和行为记录可能也会受到影响。`)
  if (!ok) return
  error.value = ''
  saving.value = true
  try {
    await store.deletePet(selectedPet.value.id)
  } catch (exception) {
    ...
    <section class="split-layout pet-editor-layout">
      <div class="panel">
        <div class="panel-title">
          <h2>选择宠物</h2>
          <span class="count-pill">{{ store.pets.length }}</span>
        </div>
        <div v-if="store.pets.length" class="pet-card-grid">
          <button
            v-for="pet in store.pets"
            :key="pet.id"
            class="pet-card"
            :class="{ selected: store.currentPetId === pet.id }"
            @click="store.setCurrentPet(pet.id)"
          >
            <div class="pet-avatar">{{ pet.name.slice(0, 1) }}</div>
            <div>
              <strong>{{ pet.name }}</strong>
              <span>{{ pet.species }} · {{ pet.breed || '未填写品种' }}</span>
              <small>{{ pet.weightKg || '-' }} kg · {{ pet.gender || '未填写性别' }}</small>
            </div>
            <Check v-if="store.currentPetId === pet.id" :size="18" />
          </button>
        </div>
      ...
      <div class="editor-column">
        <form class="panel form-panel" @submit.prevent="updatePet">
          ...
          <template v-if="selectedPet">
            ...
            <button class="danger-action" type="button" :disabled="saving" @click="removePet">
              <Trash2 :size="18" />
              删除宠物
            </button>
          </template>
        ...
        <form class="panel form-panel" @submit.prevent="createPet">
          ...
          <button class="toolbar-button" type="submit" :disabled="saving">
            <Plus :size="18" />
            创建宠物
          </button>
        </form>
